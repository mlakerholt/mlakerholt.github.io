...
export function header(base='./',current='Overview'){return `<header class="desk-header"><div class="site-crumb"><a href="/">Magnus Lian Akerholt</a><span>/</span><a href="/apps/">Apps</a><span>/</span><span>Evozyme</span></div><div class="desk-masthead"><a class="wordmark" href="${base}">Evozyme<span>.</span></a><p>Directed evolution, without the clutter</p><label class="appearance">Appearance <select id="appearance"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label></div><nav class="destinations" aria-label="Application">${[['Overview',base],['Planner',base+'planner.html#plan'],['Guide',base+'guide/index.html'],['Files',base+'files.html']].map(([name,href])=>`<a href="${href}" data-destination="${name}" ${current===name?'aria-current="page"':''}>${name}</a>`).join('')}</nav></header>`;}
export function syncNavigation(){
  const context=getContext();
  for(const a of document.querySelectorAll('[data-destination]')){const dest=a.dataset.destination;if(context&&dest==='Planner'){const url=new URL(a.href);url.search=new URL(context.url,location.href).search;url.hash=['plan','library','results','decide'].includes(context.stage)?context.stage:'plan';a.href=url.href;}}
