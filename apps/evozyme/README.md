...
Start at `overview.html` for a graphical mental map: Plan → Test → Learn, with links to all seven app stages, laboratory work, quality checks, and the next-round loop. The overview is linked from the planner and handbook, works without JavaScript, adapts to narrow screens, and includes a landscape print layout.
...
- `overview.html`, `overview.css`: process overview and its responsive/print styles; the handbook entry is generated by `scripts/build.mjs`.
