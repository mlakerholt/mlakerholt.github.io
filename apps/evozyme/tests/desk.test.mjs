...
test('guided brief requires meaningful identity, goal, units and comparison while retaining answers',()=>{
 const c=newCampaign(),r=activeRound(c);assert.ok(setupIssues(c,r,1).some(x=>x.path==='brief.parent'));r.brief.parent='LipA';r.brief.objective='Useful product rate';assert.equal(setupIssues(c,r,1).length,0);assert.ok(setupIssues(c,r,2).some(x=>x.path==='brief.units'));Object.assign(r.brief,{metric:'Rate',units:'µM/min',conditions:'Matched reference conditions'});const before=JSON.stringify(c);assert.deepEqual(setupIssues(c,r,2),[]);assert.equal(JSON.stringify(c),before);
});
