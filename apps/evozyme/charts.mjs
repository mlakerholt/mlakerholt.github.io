...
export function linePlot(series,{xLabel='Time (s)',yLabel='Signal',title='Reaction progress',height=245}={}){
  const colors=['var(--plot1)','var(--plot2)','var(--plot3)','var(--plot4)'];const all=series.flatMap(s=>s.points).filter(p=>p.every(Number.isFinite));if(!all.length)return'<p class="empty">No finite observations to plot.</p>';
  ...
  for(let i=0;i<=4;i++){const xx=xmin+(xmax-xmin)*i/4,yy=ymin+(ymax-ymin)*i/4;svg+=`<path d="M${left},${y(yy)}H${w-right}" stroke="var(--line)"/><text x="${left-8}" y="${y(yy)+4}" text-anchor="end" font-size="12">${e(Number(yy.toPrecision(3)))}</text><text x="${x(xx)}" y="${h-bottom+19}" text-anchor="middle" font-size="12">${e(Number(xx.toPrecision(4)))}</text>`;}
  svg+=`<path d="M${left},${top}V${h-bottom}H${w-right}" fill="none" stroke="var(--edge)"/><text x="${w/2}" y="${h-6}" text-anchor="middle" font-size="14">${e(xLabel)}</text><text x="16" y="${h/2}" transform="rotate(-90 16 ${h/2})" text-anchor="middle" font-size="14">${e(yLabel)}</text>`;
...
export function bars(items,title='Cost breakdown',unit='NOK'){
  ...
  return`<svg class="plot" viewBox="0 0 ${w} ${h}" role="img" aria-label="${e(title)}. Values are in the table."><title>${e(title)}</title>${items.map(([name,v],i)=>`<text x="8" y="${i*rowHeight+24}" font-size="14">${e(name)}</text><rect x="${left}" y="${i*rowHeight+9}" width="${v/max*(w-left-95)}" height="20" fill="var(--green)"/><text x="${left+v/max*(w-left-95)+7}" y="${i*rowHeight+24}" font-size="12">${e(v.toLocaleString('en-GB',{maximumFractionDigits:0}))}</text>`).join('')}<text x="${w-8}" y="${h-3}" font-size="12" text-anchor="end">${e(unit)}</text></svg>`;
}
